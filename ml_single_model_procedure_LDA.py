...
parser.add_argument('--test_run', required=False, type=str, default="",
                    help="model name")
parser.add_argument('--dataset', required=False, type=str, default="usnavy",
                    help="dataset usnavy or semeval")
parser.add_argument('--topic', required=False, default=False,
                    action='store_true', help='Fine tune pre-trained LM')
parser.add_argument('--only_topic', required=False, default=False,
                    action='store_true', help='Fine tune pre-trained LM')
parser.add_argument('--tf', required=False, default=False,
                    action='store_true', help='Fine tune pre-trained LM')
...
if dataset == "usnavy":
    ...
    df = pd.read_excel(f"./{dataset}_data/source_data/LDA.xlsx", converters={'dummy_id': str})
    ...
    df_semtest = dt.fread("./semeval_data/source_data/topic_test_data.csv").to_pandas()
    df_semtrain = dt.fread("./semeval_data/source_data/topic_train_data.csv").to_pandas()
...
dftf, allFeatures = _utils.term_frequency(train_ml=train_ml, df=df, allFeatures=allFeatures)
...
if topic:
    for n in n_comp_list:
        df_ml[f'{n}_topic'] = df[f'{n}_topic'].astype(int)
...
if term_frequency:
    all_language_models = ["Term Frequency"]
...
if _shap is None:
    ...
    for language_model in all_language_models:
        ...
        for classification_model in models:
            preds, trues = _utils.ML_classification(allFeatures=allFeatures, train_ml=train_ml, test_ml=test_ml,
                                                    ...
                                                    language_model=language_model, fold_number=fold_number, topic=topic,
                                                    n_comp_list=n_comp_list, only_topic=only_topic,
    ...
    pd.DataFrame(allPreds).to_csv(f"./{dataset}_data/{term_frequency}_{topic}_{only_topic}_{test_run}_predictions.csv")
    pd.DataFrame(allTrues).to_csv(f"./{dataset}_data/{term_frequency}_{topic}_{only_topic}_{test_run}_trues.csv")

    _utils.compute_metrics(dataset=dataset, test_run=test_run, topic=topic, only_topic=only_topic,
                           term_frequency=term_frequency)
